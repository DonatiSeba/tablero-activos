...
def _system_data(rubro: str, category: str, product: str) -> dict:
    return {
        "cells": [
...
def populate(factory: sessionmaker[Session]) -> None:
    with factory() as db:
        ...
        db.add_all((audit_a, audit_b, unresolved))
        db.flush()
        db.add_all(
            (
...
def test_read_pagination_filters_and_empty_drilldown_are_bounded(database: sessionmaker[Session]) -> None:
    ...
    assert empty.json() == {
        "cost_center": None,
        "source": {"source": "system", "available": False, "batch_id": None, "report_date": None},
        "groups": [],
        "page": {"limit": 100, "offset": 0, "has_more": False, "total_count": 0},
    }
