...
export function App() {
  ...
  if (!user) return <main className="login-shell"><section className="login-card" aria-labelledby="login-heading"><div className="login-brand"><span className="brand-mark" aria-hidden="true">T</span><div><strong>TISICO</strong><span>Control de Activos</span></div></div><h1 id="login-heading">Iniciar sesión</h1><form onSubmit={login}><label>Usuario <input name="username" autoComplete="username" required /></label><label>Contraseña <input name="password" type="password" autoComplete="current-password" required /></label><button className="button-primary">Ingresar</button></form><ErrorNotice error={sessionError} /></section></main>;
