...
- [x] AR-07 Deliver the executive and operational frontend.
  ...
  - [x] AR-07B Deliver React executive and operational views.
    ...
    - Evidence: completed and committed 2026-09-21 as `5406b1c` (`feat: deliver executive and operational views`). Vitest/RTL covers session, role affordances, freshness, error state, and multipart upload behavior; frontend build and 64 backend regression tests passed.

- [x] AR-08 Prepare operational deployment and automation handoff.
  ...
  - Checks: backend readiness/static production contract tests; offline Alembic SQL; frontend build; whitespace check. Docker Compose/live VPS checks remain manual because Docker is unavailable locally.
  - Evidence: completed 2026-09-21; no deployment, n8n modification, real secret, or PR was created. Static checks pass; Docker Compose rendering and live VPS validation remain a required pre-deployment gate.
...
- 2026-09-21: AR-08 added the isolated `activos` production Compose override, Traefik HTTPS labels, Compose-secret file contract, serialized migration operation, detail-free readiness probe, paired backup/restore scripts, and deployment/automation handoffs. No remote action or n8n change occurred; Docker/VPS validation remains a manual production prerequisite.
- 2026-09-21: VPS Compose rendering exposed an inherited `SESSION_SECRET` interpolation defect before production file-secret overrides could apply. The focused correction removes direct-secret requirements from base interpolation, preserves strict direct/file conflict checks, and passed 76 backend tests; live VPS Compose rendering remains required.

## Next step
Run the documented one-time VPS preflight and first release with an approved operator, including a restore rehearsal before relying on scheduled backups.
